...
# ★v1.2：重い処理を別スレッドで走らせるために必要。
# COMは「スレッドごとに初期化する」決まりなので、ワーカースレッドの中で
# CoInitialize() を呼ばないと PowerPoint を掴めない。
import pythoncom
...
def extract_terms(file_path, override_dict, include_title=False):
    ...
    for surface, reading_form, _ in tokenize_all(full_text):
        ...
        # 字下げやレイアウトに全角スペースを使ったスライドでは、Sudachiが
        # 空白の連なり（例："\n　　　　"）を1つのトークンとして返す。
...
# 本文が動いたとみなす下限（pt）。再レイアウト後の測定には小さな誤差が
# 乗るので、1pt未満は「動いていない」として扱う。
LAYOUT_SHIFT_THRESHOLD_PT = 1.0


def text_block_height(shape):
    """シェイプ内のテキスト全体の高さ(pt)。取れなければ None。

    ルビを置くために行間を広げると、この高さが伸びる＝2行目以降の文字が
    下へ動く。ボックス自体の位置（Left/Top）は変わらないため、
    「本文の空欄に別のテキストボックスを重ねてアニメーションで表示する」
    ような作り方をしているスライドでは、本文だけがズレて見える。
    それを検出するための測定。"""
    try:
        return float(shape.TextFrame.TextRange.BoundHeight)
    except Exception as e:
        logging.debug(f"テキスト高さの取得に失敗しました: {e}")
        return None
...
def process_shape(slide, shape, term_dict, settings, measurer, refresh, warnings,
                  used=None, shifted_slides=None):
    """1つのシェイプにルビを振り、グループ化とアニメーション継承まで行う。
    used は「初回のみ」モードで既にルビを振った表層形の集合（資料全体で共有）。
    shifted_slides は「本文の位置が動いたスライド番号」の集合（v1.2で追加）。"""
    if used is None:
        used = set()
    if shifted_slides is None:
        shifted_slides = set()
    try:
    ...
    shifted = False
    try:
        # ★v1.2：ルビを置くために本文がどれだけ動くかを測る。
        # 自動調整の解除と行間の拡張はどちらも文字位置を動かすので、
        # その両方を行う「前」の高さを控えておく。
        height_before = text_block_height(shape)
        ...
        # ★v1.2：本文が下へ伸びた分だけ、2行目以降の文字が動いている。
        # 位置を自動で補正することはしない（対応関係の情報がファイルに
        # 無いため、他の図形を動かすと誤爆する）。動いた事実だけを
        # 利用者へ伝えて、目で確認してもらう。
        # ★ここではまだ shifted_slides へ入れない。この後ルビを1件も
        #   置けなかった場合は restore_parent_shape で行間を元へ戻すので、
        #   ファイル上はズレが残らない。それなのに警告だけ出ると、
        #   利用者は「動いていないスライド」を延々と確認させられる。
        #   ルビの配置が成功したことを確かめてから記録すること。
        height_after = text_block_height(shape)
        shifted = (
            height_before is not None
            and height_after is not None
            and height_after - height_before > LAYOUT_SHIFT_THRESHOLD_PT
        )
    ...
    if not ruby_boxes:
        ...
        # 巻き戻した以上ズレは残らないので、警告も出さない（shifted は捨てる）。
        try:
            restore_parent_shape(slide, shape)
        except Exception as e:
    ...
    # ここまで来たらルビは実際に配置されている＝広げた行間はファイルに残る。
    # このスライドは本文が動いた可能性があるものとして記録する。
    if shifted:
        try:
            shifted_slides.add(int(slide.SlideIndex))
        except Exception as e:
            logging.debug(f"スライド番号の取得に失敗しました: {e}")
...
def generate_ruby_pptx(pptx_path, term_dict, settings, ppt_app=None, warnings=None,
    ...
    ★v1.2：on_progress は「(現在のスライド番号, 総スライド数)」を受け取る
    関数。処理に時間がかかっているとき、実行中なのか固まったのかを
    利用者が判断できるようにするために呼ぶ。
    ...
    try:
        ...
        # ★v1.2：ルビの分だけ本文が下へ動いたスライドの番号。
        # スライドごとに1件へ集約してから警告に出す。シェイプごとに
        # 積むと、警告欄の表示上限（先頭10件）を1枚のスライドで
        # 使い切ってしまい、他の警告が埋もれる。
        shifted_slides = set()

        slide_total = pres.Slides.Count
        for index in range(1, slide_total + 1):
            if on_progress is not None:
                on_progress(index, slide_total)
            ...
            for shape in targets:
                try:
                    total_ruby += process_shape(
                        ...
                        used=used_surfaces, shifted_slides=shifted_slides,
                    )
                except Exception as e:
                    logging.error(f"スライド{index}のシェイプ処理でエラー: {e}")
                    warnings.append(f"スライド{index}: {e}")

        # ★v1.2：本文が動いたスライドを1件にまとめて知らせる。
        # 位置の自動補正はしない。「どの図形がどの行に対応するか」という
        # 情報がPowerPointのファイルに存在せず、座標の重なりから推測して
        # 動かすと誤爆するため。動いた事実だけを伝えて目視に委ねる。
        if shifted_slides:
            numbers = "、".join(str(n) for n in sorted(shifted_slides))
            warnings.append(
                f"スライド {numbers}：ルビを入れる分だけ本文の位置が動いた可能性があります。"
                "他の図形との重なりを確認してください。"
            )
...
class RubyEditorApp:
    ...
    def __init__(self, root):
        ...
        # 別スレッドで処理中かどうか（v1.2で追加）。詳細は _reject_if_busy を参照。
        self._busy = False
        self.data = []
        self.override_dict = {}
    ...
    def _reject_if_busy(self):
        ...
        投げ込まれると2つ目のワーカースレッドが起動し、同じPowerPointを
        2つのスレッドから同時に操作することになる。
        v1.1 はメインスレッドを塞いでいたので起こり得なかったが、
        v1.2 でバックグラウンド化した副作用として生じた穴なので、
        グラブに頼らず明示的なフラグで塞ぐ。"""
    ...
    def _run_in_background(self, work, on_success, on_error,
        ...
        ★なぜスレッドが要るのか
          ルビの配置はスライドとシェイプの数だけCOM呼び出しを繰り返す。
          これをメインスレッドで回すと tkinter が画面を描き直せなくなり、
          ウィンドウが白く固まって「応答なし」になる。実行中なのか
          壊れたのか利用者に区別が付かない、というのが v1.1 までの問題。

        ★COMの制約
          COMのオブジェクトはスレッドをまたいで使えない。そのため
          ワーカースレッドの中で CoInitialize() を呼び、PowerPointの取得も
          そのスレッドの中で行う（work の中で get_powerpoint() を呼ぶ）。
          メインスレッドで掴んだPowerPointをワーカーへ渡してはいけない。

        ★UIはメインスレッドから
          tkinter はスレッドセーフではないので、ワーカーからは画面を
          触らない。結果はキューに入れ、メインスレッド側の poll() が
          取り出して on_success / on_error を呼ぶ。
    ...
    def _process_batch_files(self, pptx_files):
        ...
        def work():
            ...
            try:
                for i, file_path in enumerate(pptx_files, start=1):
                    self._batch_state = {
                        "index": i, "name": file_path.name, "slide": 0, "slides": 0,
                    }

                    def on_slide(index, slides, _i=i, _name=file_path.name):
                        self._batch_state = {
                            "index": _i, "name": _name, "slide": index, "slides": slides,
                        }
                    ...
                    try:
                        ...
                        _, count = generate_ruby_pptx(
                            file_path, dict(term_pairs), settings, ppt_app=ppt,
                            warnings=warnings, on_progress=on_slide,
                        )
                        if count == 0:
        ...
        def on_success(payload):
            success_count, failures = payload
            finish()
    ...
    def run_single_file_output(self):
        ...
        self._slide_state = {"slide": 0, "slides": 0}

        def on_slide(index, slides):
            self._slide_state = {"slide": index, "slides": slides}

        def work():
            return generate_ruby_pptx(
                self.current_file_path, dict(self.data), settings,
                warnings=warnings, on_progress=on_slide,
            )

        def progress_source():
            state = self._slide_state
            if state["slides"] <= 0:
                return None
            return (f"スライド {state['slide']}/{state['slides']}",
                    state["slide"], state["slides"])

        def on_success(payload):
            out_path, count = payload
        ...
        def on_error(exc):
            logging.error(f"ルビ付きPowerPoint出力に失敗しました: {exc}")
            msgbox.showerror(
                "出力エラー",
    ...
    def advance_to_next(self):
        ...
        def work():
            return generate_ruby_pptx(
                current_file, dict(self.data), settings,
                warnings=warnings, on_progress=on_slide,
            )
...
if __name__ == "__main__":
    ...
    try:
        ...
        # ★v1.2：処理中に「×」で閉じられないようにする。
        # v1.1 は処理がメインスレッドを塞いでいたため、そもそも閉じる操作が
        # できなかった。v1.2 は別スレッドで処理を回すのでメインループが
        # 生きており、閉じられてしまう。ワーカーは daemon なので
        # プロセスと一緒に強制終了され、PowerPointの終了処理（Quit）や
        # 作業用コピーの後片付けが走らないまま残骸が残る。
        def on_close():
