...
'   word.Run("InsertFuriganaFromTSV_V30", 元ファイル, TSVファイル, 出力ファイル)
'
' ★v2.1 からの主な変更点
'  1) 表の中を検索したときに空回りする不具合を修正した。
'     Word の Range.Find は、検索している位置が表のセルの中にあると、
'     Wrap:=wdFindStop を指定していても「検索開始位置より前にある一致」を
'     見つけたと返してくることがある（1×1の表だけを置いた文書で再現を確認。
'     表が無い文書では起きない）。v2.1 はこれをそのまま新しいヒットとして
'     受け取っていたため、既に処理済みの同じ場所を延々と拾い直し、
'     MAX_HITS_PER_TERM（20000回）に達するまで空回りしていた。
...
'     Python はこれを読んで「語句を検索中 45/81」のように表示する。
'
' ★v2.0 からの主な変更点（v2.1 で対応済み）
...
Public Sub InsertFuriganaFromTSV_V30(ByVal srcPath As String, _
...
Private Sub InsertAllGuides(ByVal docNew As Document, _
    ...
    ' 進捗ファイルを書き出す間隔。毎回書くと語句数の多い文書では
    ' ファイルI/Oだけで無視できない時間を食うので間引く。
    ...
    ' ======== 第1段階：位置決め（文書は変更しない） ========
    progressStep = terms.Count \ 100
    If progressStep < 1 Then progressStep = 1
    WriteProgress tsvPath, "search", 0, terms.Count
    ...
    For idx = 1 To terms.Count
        ...
        If Len(targetWord) > 0 And Len(furigana) > 0 Then
            ...
            Do
                ...
                ' ★重要：検索している位置が表のセルの中にあると、Word の
                '   Range.Find は Wrap:=wdFindStop を指定していても、
                '   検索開始位置より前にある一致を「見つけた」と返してくる。
                '   （1×1の表だけを置いた文書で再現を確認済み。）
                '   そのまま信じると、既に occupied 済みの同じ場所を延々と
                '   拾い直し、MAX_HITS_PER_TERM まで空回りする。v2.1 で
                '   「表を含む文書だけ処理に数分かかる」原因になっていた。
                '   前に戻っていたら、もうこの先に一致は無いと判断して抜ける。
                If hitStart < searchStart Then Exit Do
                searchStart = hitEnd
        ...
        If (idx Mod progressStep) = 0 Or idx = terms.Count Then
            WriteProgress tsvPath, "search", idx, terms.Count
        End If
    Next idx
    ...
    processed = 0
    progressStep = matches.Count \ 100
    If progressStep < 1 Then progressStep = 1
    WriteProgress tsvPath, "apply", 0, matches.Count
    ...
    For pos = docLen To 0 Step -1
        If startOf(pos) > 0 Then
            ...
            If (processed Mod progressStep) = 0 Or processed = matches.Count Then
                WriteProgress tsvPath, "apply", processed, matches.Count
            End If
        End If
    Next pos
End Sub
...
'------------------------------------------------------------
' 途中経過を TSV と同じ場所へ書き出す（拡張子 .progress、ASCIIのみ）。
' Python 側は別スレッドからこれを読んで進捗表示を更新する。
'
' ★書き込み中は Word がこのファイルを排他ロックするため、Python 側の
'   読み取りが一時的に失敗することがある。Python 側は失敗を無視して
'   次の周期で読み直す作りにしてある。
' ★phase は "search"（第1段階＝位置決め）か "apply"（第2段階＝ルビ付与）。
'   Open ... For Output は ANSI 固定なので ASCII のみを書く。
'------------------------------------------------------------
Private Sub WriteProgress(ByVal tsvPath As String, _
    ...
    On Error Resume Next
    progressPath = tsvPath & ".progress"
    fileNum = FreeFile
    Open progressPath For Output As #fileNum
    Print #fileNum, "phase" & vbTab & phase
    Print #fileNum, "current" & vbTab & CStr(current)
    Print #fileNum, "total" & vbTab & CStr(total)
    Close #fileNum
    On Error GoTo 0
End Sub
...
'------------------------------------------------------------
' 進捗ファイルを片付ける。成功・失敗のどちらの経路でも必ず通ること。
' 残したままにすると、次回の実行で前回の件数が一瞬表示される。
'------------------------------------------------------------
Private Sub DeleteProgress(ByVal tsvPath As String)
    Dim fso As Object

    On Error Resume Next
    Set fso = CreateObject("Scripting.FileSystemObject")
    If fso.FileExists(tsvPath & ".progress") Then
        fso.DeleteFile tsvPath & ".progress", True
    End If
    On Error GoTo 0
End Sub
